...
//
//    Rect leftBr = boundingRect(maskLeft);
//    Rect rightBr = boundingRect(maskRight);
//    Rect uni = leftBr | rightBr;
//
//    rectangle(left, leftBr.tl(), leftBr.br(), Scalar(0, 255, 255), 2, LINE_AA);
//    rectangle(right, rightBr.tl(), rightBr.br(), Scalar(0, 255, 255), 2, LINE_AA);
//
//    std::cout << "left: " << (double)leftBr.x / 640 << " " << (double)leftBr.y / 480 << " " << (double)(leftBr.x + leftBr.width) / 640 << " " << (double)(leftBr.y + leftBr.height) / 480 <<std::endl;
//    std::cout << "right: " << (double)rightBr.x / 640 << " " << (double)rightBr.y / 480 << " " << (double)(rightBr.x + rightBr.width) / 640 << " " << (double)(rightBr.y + rightBr.height) / 480 <<std::endl;
//    std::cout << "uni: " << (double)uni.x / 640 << " " << (double)uni.y / 480 << " " << (double)(uni.x + uni.width) / 640 << " " << (double)(uni.y + uni.height) / 480 <<std::endl;
//
//    rectangle(left, uni.tl(), uni.br(), Scalar(0, 0, 255), 2, LINE_AA);
//    rectangle(right, uni.tl(), uni.br(), Scalar(0, 0, 255), 2, LINE_AA);
//
//    imshow("left", left);
//    imshow("right", right);
//
//    imwrite("/media/balint/Data/Linux/diploma/magic1.png", left);
//    imwrite("/media/balint/Data/Linux/diploma/magic2.png", right);
//
//    waitKey();
//
//    return 0;
...
int main(int argc, char **argv) {
    ...
    Ptr<Camera> camera1(new RealCamera(1, "intrinsics_left.yml"));
    CameraPoseCalculator calculator1(camera1);
    Ptr<Camera> camera2(
            new RealCamera(2, "intrinsics_right.yml"));
...
//            drawGridXY(image2, camera2, calculator2.cameraPose);
        ...
        if (ch == 27) {
            ...
            if (calculator1.calculate() && calculator2.calculate()) {
                ...
                imwrite("polc_left.png", image1);
                imwrite("polc_right.png", image2);

                calculator1.cameraPose->save("polc_pose_left.yml");
                calculator2.cameraPose->save("polc_pose_right.yml");
...
//};
//
//int mainSDFSDF(int argc, char **argv) {
//
//    vector<Ptr<Camera>> camera(2);
//    camera[Camera::LEFT] = Ptr<Camera>(
//            new RealCamera(Camera::LEFT, "/media/balint/Data/Linux/diploma/src/final/intrinsics_left.yml"));
